        ...
        if (os.path.isfile("./trigger.dat")):
            ...
            if len(trigger_input['mac'])>0:
                if len(sent_already["mac"])!=0:
                    ...
                    trig_unique_1 = np.array([x if ((x not in sent_already['mac'])|( (x in sent_already['mac'])&(  np.max(sent_already['sent_time'][sent_already['mac']==x]) < calendar.time$
                ...
                if len(trig_unique_2)>0:
                    ...
                    for x in trig_unique_2:
                        if x not in restricted_list:
                            ...
                            print_message = "Mac:%s, Last Visited:%s, Time Spent:%s minutes \n"%(x, time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(trigger_input['last_seen'][trigger_input[$
                            print print_message
