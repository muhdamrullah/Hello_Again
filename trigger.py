        ...
        if (os.path.isfile("./trigger.dat")):
            ...
            if len(trigger_input['mac'])>0:
                if len(sent_already["mac"])!=0:
                    trig_unique_1 = np.array([x if ((x not in sent_already['mac'])|( (x in sent_already['mac'])&(  np.max(sent_already['sent_time'][sent_already['mac']==x]) < calendar.timer.timegm(time.gmtime()) - 5*60))) else "-99" for x in trigger_input['mac']])
